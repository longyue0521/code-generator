...
func Test_parseServiceDefinition(t *testing.T) {
	...
	tests := []struct {
		name    string
		typ     annotation.ASTType
		want    template.ServiceDefinition
		wantErr error
	}{
		{
			/*
			...
			*/
			...
			want: template.ServiceDefinition{
				Package: pkg,
				Name:    "UserService",
				Methods: []template.ServiceMethod{
					{
						Name:         "Get",
						Path:         "/Get",
			...
			*/
			...
			typ: annotation.ASTType{
				Group: annotation.Group[*ast.TypeSpec]{
					...
					List: []annotation.Single{{"ServiceName", "MyUserService"}},
				},
				Fields: []annotation.ASTField{
					{
						Group: annotation.Group[*ast.Field]{
							...
							List: []annotation.Single{{Key: "Path", Value: "/user"}},
						},
					},
				},
			},
...
func Test_gen(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.NoError(t, os.Remove(tc.genFile))
		})
	}
...
func Test_parseFiles(t *testing.T) {
	...
	assert.Equal(t, []template.ServiceDefinition{
		{
			Package: "testdata",
			Name:    "MyOrderService",
			Methods: []template.ServiceMethod{
